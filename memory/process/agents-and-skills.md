...
**Урок 2026-08-03 (стоил ~2M токенов):** агенты Workflow без явной `model:` наследуют модель ОСНОВНОЙ сессии — на Fable это ~70k токенов на файл против ~25k у sonnet при том же качестве механической работы. Массовые прогоны (аудит по файлу на агента, переводы, верификация) — ВСЕГДА `model: 'sonnet'` явно; дорогую модель оставлять только синтезу/судьям, и то осознанно. Прогон 317 агентов на дефолте упёрся в лимит сессии на 50-м; кэш resume ключуется парой (промпт, опции), так что смена модели его обнуляет — закладывай модель ДО старта, а не после.

**Стандарт больших фич (указания юзера 2026-08-03/04):**
1. СНАЧАЛА разведка отдельным запуском, план и скрипт волны строятся ПО ЕЁ ДАННЫМ — не из головы оркестратора. Захардкоженный список целей уже стоил проёба (реликвии выпали из раздачи пассивок: оркестратор перечислил weapons/armor/accessories по памяти, 2026-08-04). Инвентарь целей — работа разведчика, не оркестратора.
2. Оркестратор сам код НЕ пишет — раздаёт задания агентам с урезанным системным промптом (Workflow).
3. Модель — ПО СЛОЖНОСТИ задачи: дизайн/баланс/тонкая механика — модель сессии; механика по спеке/переводы — sonnet; тривиальный merge — sonnet low. ОСТОРОЖНО с «контентом»: раздача с перекрёстными квотами (разнообразие×тематика×уникальность на 50+ единицах) — это комбинаторика, sonnet на ней молотит по кругу и не пишет артефакт (weapons/armor, 42 мин впустую, 2026-08-04) — таким давать модель сессии + указание «план таблицей до записи, файл пишется один раз». Признак зависания: транскрипт растёт, артефакт не обновляется.
4. После волны обязательны verify + reviewer и отчёт юзеру.
