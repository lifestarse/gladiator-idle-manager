...
**Стандарт больших фич (указание юзера 2026-08-03):** оркестратор сам код НЕ пишет — раздаёт задания агентам с урезанным системным промптом (Workflow); модель выбирается ПО СЛОЖНОСТИ задачи (дизайн/баланс/тонкая механика — модель сессии, механика по спеке/контент/переводы — sonnet, тривиальный merge — sonnet low); после волны обязательны verify + reviewer и отчёт юзеру.
