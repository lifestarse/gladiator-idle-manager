...
**Реальное (совпадает с [[audit-2026-07-24]]):** ~~16 broad `except Exception`~~ и ~~11 `print()`~~ (оба ЗАКРЫТЫ, см. ниже), функции 120–156 строк (`_player_attack_phase`, `_enemy_attack_phase`, `_apply_save_data`), вложенность до 8 (`_restore_android`), 69 «repetitive dispatch ladder», 17 chained `.get(..., {})`.
