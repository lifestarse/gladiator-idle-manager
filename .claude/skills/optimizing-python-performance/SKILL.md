...
> (`skills/python/performance`), MIT — текст лицензии рядом в `LICENSE`.
> Установлено 2026-07-28.
>
> **Поправка на этот проект.** Скилл написан про библиотеки на uv; здесь ни uv, ни
> pytest-benchmark нет — зависимости в `requirements.txt`, python 3.10, запуск
> `python -m pytest`. Свои замеры у проекта уже есть: `tests/test_battle_speed.py`
> и `tests/test_bench.py`. Читать ради инструментов профилирования (cProfile,
> PyInstrument, memray, tracemalloc) и порядка действий, а не ради команд `uv run`.
> Узкое место здесь обычно не в Python-логике, а в перерисовке Kivy на устройстве —
> прежде чем оптимизировать формулу, докажи профилем, что дело в ней.
