...
- [Удалённые шрифты 2026-07-27](memory/remote-fonts-2026-07-27.md) — шрифт едет с языковым паком (content-addressed по sha256), список языков в пикере из манифеста: новый язык без релиза; грабли — IncompleteRead, kivy парсит argv
...
- [Сигналы codemap 2026-07-27](memory/codemap-signals-2026-07-27.md) — чему в codemap верить нельзя: `# Build: N` не счётчик правок (наследуется при сплите), `from . import X` рождает фантомные циклы, «неупомянутый» артефакт может грузиться вычисляемым путём
- [Циклы импортов 2026-07-27](memory/import-cycles-2026-07-27.md) — 3 из 4 разорваны, `localization ↔ remote_content` намеренный; ловушка: импорт сабмодуля НЕ развязывает с пакетом (родительский `__init__` выполняется целиком)
...
- [Агенты и скиллы 2026-07-28](memory/agents-and-skills-2026-07-28.md) — 8 субагентов в .claude/agents/ (verify/scout/factcheck/reviewer/buildcheck/testwright/i18n-translate/i18n-qa); из сторонних скиллов подошли только 2, остальные проверены и отвергнуты с доказательством
